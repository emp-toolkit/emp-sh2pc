...
static bool brev(const SH2PCSession::Bit& b) { return g_ctx->reveal(b, PUBLIC).value(); }
...
void fp_if(double a, double b) {
	...
	F x = finput(ALICE, (float)a), y = finput(BOB, (float)b);
	SH2PCSession::Bit one = SH2PCSession::Bit::constant(g_ctx->ctx(), true);
	SH2PCSession::Bit zero = SH2PCSession::Bit::constant(g_ctx->ctx(), false);
