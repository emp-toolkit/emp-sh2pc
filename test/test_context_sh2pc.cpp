// Phase B1: NATIVE semi-honest Session/Context migration. A BooleanContext-
// templated circuit runs on the garbled 2PC backend through SH2PCSession +
// SH2PCContext (sh2pc_session.h) — no global emp::backend, no Backend virtual
// dispatch; AND gates go straight to halfgates_garble/eval, and typed
// input<T>()/reveal<T>() own the OT. Demonstrates a keep-templated integer
// kernel and an IR-replay float builtin. C++20.

#include "emp-sh2pc/emp-sh2pc.h"          // NetIO, parse_party_and_port, ALICE/BOB
#include "emp-sh2pc/sh2pc_session.h"      // native SH2PCSession / SH2PCContext
...
int main(int argc, char** argv) {
    ...
    // 1) keep-templated kernel: UInt32 add (ALICE owns a, BOB owns b).
    {
        ...
        auto a = sess.input<UInt<SH2PCContext, 32>>(ctx, ALICE, (uint64_t)av);
        auto b = sess.input<UInt<SH2PCContext, 32>>(ctx, BOB,   (uint64_t)bv);
        auto c = a + b;
    ...
    {
        auto a = sess.input<Float<SH2PCContext, 32>>(ctx, ALICE, 1.5f);
        auto b = sess.input<Float<SH2PCContext, 32>>(ctx, BOB,   2.25f);
        auto c = a + b;
    ...
    // 3) PUBLIC input is a public constant (no OT): a + b + PUBLIC(k).
    {
        ...
        auto a = sess.input<UInt<SH2PCContext, 32>>(ctx, ALICE,  (uint64_t)av);
        auto b = sess.input<UInt<SH2PCContext, 32>>(ctx, BOB,    (uint64_t)bv);
        auto k = sess.input<UInt<SH2PCContext, 32>>(ctx, PUBLIC, (uint64_t)kv);
        uint32_t r = (uint32_t)sess.reveal(a + b + k, PUBLIC);
        if (party == BOB) {
    ...
    sess.finalize();
    if (party == BOB) printf("test_context_sh2pc (native session/context): %s\n", fails ? "FAILED" : "PASS");
    return fails ? 1 : 0;
}
