...
//                  finalize(). sess.direct_ctx() is the gate context values are
//                  built over. input/reveal are generic over any WireValue — the
//                  session names no value family.
//   SH2PCCtx     — the direct gate context (SH2PCSession::DirectCtx). A
...
// UInt_T<SH2PCSession::DirectCtx, N> (etc.) over sess.direct_ctx(). No global backend.
